    ...
    $(document).ready(function(){
               //提交
                $(".sub").click(function(){
                    ...
                    t = "post";
                    u = "HFSql.php";
                    d = $("#" + formID).serialize() + "&btn=" + btnName + "&type=HFFee";
                    dt = "json";
                    m = function(json){
                        var i, n, k, v;
...
<div id="main">
    {if $smarty.get.query neq 'detail'}
        <form id="HFBalFeeForm" name="HFBalFeeForm">
            <input type="hidden" name="unitID" value="{$smarty.get.unitID}">
            <input type="hidden" name="month" value="{$smarty.get.month}">
            <input type="hidden" name="HFDate" value="{$smarty.get.HFDate}">
            <input type="button" class="sub" name="HFBalFeeBtn" value="提交平账结果">
            ...
            <table class="myTable">
                ...
                <tbody>
                    {foreach from=$margin item=val key=k}
                        <tr>
                            <td>{$k}</td>
                            <td><a href="{$httpPath}workerInfo/wManage.php?uID={$k}" target='_blank'>{$HFR[$k].name|default:$ofR[$k].name}</a></td>
                            <td>{$HFR[$k].pTotal}</td>
                            <td>{$HFR[$k].uTotal}</td>
                            <td>{$ofR[$k].pHF}</td>
                            <td>{$ofR[$k].uHF}</td>
                            <td>{if $val.pMargin}<input type=text name="pHFMoney[{$k}]" value="{$val.pMargin}" size="5" />{/if}</td>
                            <td>{if $val.uMargin}<input type=text name="uHFMoney[{$k}]" value="{$val.uMargin}"  size="5" />{/if}</td>
                        </tr>    
                     {/foreach}   
                </tbody>
                </thead>
