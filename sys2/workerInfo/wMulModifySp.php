    ...
    public function sql() {
        ...
        foreach ($cellArray as $cValue) {
            ...
            if (!is_null($cValue ['dimissionDate'])) {
                $s_V .= "('" . $cValue ['uID'] . "','" . $cValue ['mountGuardDay'] . "','" . $cValue ['unitID'] . "','" . $cValue ['dimissionDate'] . "','" . $cValue ['dimissionReason'] . "','" . $cValue ['dimissionRemarks'] . "' ),";
            }
               //生成更新语句
        ...
        $sqlV = rtrim($s_V, ",");
        //生成插入额外的sql语句
        $dimiSql = "insert into a_dimission (uID, entryDate,lastUnitID,dimissionDate,dimissionReason,dimissionRemarks) values";
