...
/*
 * 2011-06-08
 * 社保平账审核, 这里呢, 是新版本的,:  通过 a_editAccountList , a_prsRequireMoney运算,直接得到结果,然后呢, 能从显示的页面上修改,并通过审核的
 * 
 */
#页面访问权限
...
if ($conRet) {
	...
	$updateSql = "update `a_prsRequireMoney` set ";
						...
						if ($v != 0) {
							if ($exACRet [$key] [$k] >= 0) {
								...
								if ($exRERet && $exRERet [$key] [$k] < 0) {
									//判断当月如果在社保未进行平账前,既存在挂账又有欠款..这肯定是出错了!!!;
									$errMsg [] = "员工编号为{" . $key . "}的员工社保平账出现问题..请及时查证..(该员工存在个人社保欠/挂问题)";
								}
								$v = $v + $acVal;
								if ($v >= 0) {
									$type = 1;
...
$reSql = "select a.ID,a.uID,b.name,a.uPDInsMoney,a.uSoInsMoney,a.pSoInsMoney,a.managementCostMoney,a.uAccount,a.uOtherMoney,a.pOtherMoney,a.type from `a_prsRequireMoney` a left join `a_workerInfo` b on a.uID = b.uID where a.month like :month and a.unitID like :unitID and ( a.uSoInsMoney<>0 or a.pSoInsMoney<>0 or a.uPDInsMoney<>0 or a.managementCostMoney<>0 or a.uAccount<>0 or a.pOtherMoney<>0 or a.uOtherMoney<>0) and  b.uID is not null  ";
#这里就做个EXCEL筛选模式..
...
$smarty->assign ( array (
		"uPDInsMoneyArr" => $uPDInsMoneyArr,
		"uSoInsMoneyArr" => $uSoInsMoneyArr,
		"pSoInsMoneyArr" => $pSoInsMoneyArr,
		"managementCostMoneyArr" => $managementCostMoneyArr,
		"soInsCardMoneyArr" => $soInsCardMoneyArr,
		"residentCardMoneyArr" => $residentCardMoneyArr,
		"uAccountArr" => $uAccountArr,
		"typeArr" => $typeArr 
) );
...
$smarty->display ( "approval/soInsBalFeeApproval.tpl" );
?>
