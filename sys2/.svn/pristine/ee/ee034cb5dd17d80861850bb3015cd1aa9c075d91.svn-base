...
class IndexController extends HomeController {
	...
	// 定时任务调用入口
	function cron() {
		// 定时发送模板消息 TODO 待在服务器上配置定时任务
		$map ['is_send'] = 0;
		$list = M ( 'template_msg' )->where ( $map )->limit ( 5 )->select ();
		foreach ( $list as $vo ) {
			$param = unserialize ( $vo ['param'] );
			$res = D ( 'Common/TemplateMessage' )->_replyData ( $vo ['uid'], $param, $vo ['templateID'], $vo ['url'], DEFAULT_TOKEN );
			
			$save ['sendTime'] = date ( 'Y-m-d H:i:s', NOW_TIME );
			$save ['is_send'] = 1;
			$save ['msg'] = $res ['msg'];
			$map2 ['id'] = $vo ['id'];
			M ( 'template_msg' )->where ( $map2 )->save ( $save );
		}
	}
	...
	function mytest() {
		dump ( 'it is begin' );
		$param ['fid'] = 5;
		$res = agentDataByPost ( 'agentUser', $param );
		dump ( $res );
		dump ( 'it is end' );
	}
	function mytest2() {
