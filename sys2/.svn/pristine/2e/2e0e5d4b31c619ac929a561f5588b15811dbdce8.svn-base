...
class wMulModifySoIns {
	...
	public function set() {
		...
		$table = "a_workerInfo";
		$field = $this->field;
		$fieldHeader = '<div style=""><table class="myTable"><thead><tr style="text-align:center;">
	  <th>在职状态</th><th>员工编号</th><th>姓名</th><th>身份证号码</th><th>档案编号</th><th>工资账号</th>
	  <th>曾用工资账号</th><th>性别</th><th>民族</th><th>家庭地址</th><th>工作地址</th>
	  <th>学历</th><th>政治面貌</th><th>婚否</th><th>移动电话</th><th>固定电话</th><th>联系人</th><th>联系电话</th><th>学校</th><th>开户银行</th><th>员工类型</th>
	  <th>单位编号</th><th>分公司</th><th>部门</th><th>岗位</th><th>入职日期</th><th>合同开始日期</th><th>合同终止日期</th><th>是否购买社保</th>
	  <th>户籍类型</th><th>投保日期</th><th>社保号</th><th>缴交基数</th><th>养老</th><th>医疗</th><th>工伤</th><th>失业</th><th>残障金</th><th>利手</th>
	  <th>是否购买公积金</th><th>公积金启用日期</th><th>个人公积金账号</th><th>个人比例(公积金)</th><th>单位比例(公积金)</th><th>基数(公积金)</th><th>配偶姓名</th><th>配偶身份证</th><th>购买商保</th><th>参加互助会</th><th>管理费</th>
	  <th>数码图像号</th><th>避孕节育报告单号</th><th>职称</th><th>技能等级</th><th>备注</th><th>离职日期</th><th>离职原因</th><th>离职备注</th></tr></thead>';
				...
				foreach ( $val as $k => $v ) {
					...
					switch ($k) {
						...
						case "soInsurance" :
							...
							if (! is_null ( $v ) && ! is_null ( $val ['soInsBuyDate'] )) {
			...
			foreach ( $val as $k => $v ) {
				switch ($k) {
					...
					case "PDIns" :
						...
						if ($val ['soInsBuyDate'] && ($val ['soInsurance'] != "0" || $val ['status'] != "0")) {
					...
					case "pHFPer":
					case "uHFPer":	
						if ($val['housingFund'] == "0" || $val ['status'] == "0") {
							$v = '0';
							$val ['HFModifyDate'] = $today;
						}
						if ($val ['HFBuyDate'] && $val ['housingFund'] != "0" ) {
...
//			$newCellArray [$key] ['soInsModifyDate'] = $val ['soInsModifyDate'];
	...
	public function sql() {
		...
		foreach ( $cellArray as $cValue ) {
			...
			foreach ( $cValue as $cK => $cV ) {
				if ($cK != "dimissionDate" && $cK != "dimissionReason" && $cK != "dimissionRemarks" && $cK != "uID" && $cK != "name") {
					if (! is_null ( $cV )) {
						$sqlX .= $cK . " = '" . $cV . "',";
					}
				}
			...
			if (! is_null ( $cValue ['dimissionDate'] )) {
				$s_V .= "('" . $cValue ['uID'] . "','" . $cValue ['dimissionDate'] . "','" . $cValue ['dimissionReason'] . "','" . $cValue ['dimissionRemarks'] . "' ),";
			}
			//生成更新语句
			$sqlX = rtrim ( $sqlX, "," );
			$iSql [] = "insert into a_workerInfo_history select * from a_workerInfo where uID like '" . $cValue ['uID'] . "'";
			$sql [] = $sqlH . $sqlX . " where uID like '" . $cValue ['uID'] . "'";
		...
		$sqlV = rtrim ( $s_V, "," );
		$dimiSql = "insert into a_dimission (uID, dimissionDate,dimissionReason,dimissionRemarks) values";
		$dimiSql = $dimiSql . $sqlV;
		$statusSql1 = "update a_workerInfo set status = '1' where radix <> 0 and bID <> '' and sID <> ''  and pID <> '' and type like '1' and uID in (" . $uIDV . ")";
		$statusSql2 = "update a_workerInfo set status = '1' where radix <> 0 and  sID <> ''  and pID <> '' and type not like '1' and uID in (" . $uIDV . ")";
		//生成历史记录
		...
		//生成插入额外的sql语句
		$sql ['extra'] [] = $dimiSql;
		$sql ['extra'] [] = $statusSql1;
		$sql ['extra'] [] = $statusSql2;
		return $sql;
	}
	...
	public function dataInfo() {
		...
		if ($w1 != 0) {
			$insertInfo .= '<br/>
		<div id="pilianggengxin">1. 全日制员工有' . $w1 . ' 人,有 ' . $x1 . ' 人停止购买社保,有' . $y1 . '  人停止购买商保,有' . $z1 . '  人离职<br/></div>';
			if ($x1)
				$insertInfo .= '<div id="dadiv"><div class="xiaodiv"><br><div class="wenzi">停止购买社保名单:</div>
		<div style=""><table class="myTable"><thead><tr style="text-align:center;">
	    ...
	    ' . $o1 . '
			...
			if ($y1)
				$insertInfo .= '<div class="xiaodiv"><br><div class="wenzi">停止购买商保名单:</div>
		<div style=""><table class="myTable"><thead><tr style="text-align:center;">
	    <th>姓名</th><th>编号</th></tr>
	    </thead>
	    <tbody>
	    ' . $p1 . '
			...
			if ($z1)
				$insertInfo .= '<div class="xiaodiv"><br><div class="wenzi">离职员工名单:</div>
		<div style=""><table class="myTable"><thead><tr style="text-align:center;">
	    <th>姓名</th><th>编号</th></tr>
	    </thead>
	    <tbody>
		...
		if ($w2 != 0) {
			$insertInfo .= '<br/>
		<div id="pilianggengxin">2. 非全日制员工有 ' . $w2 . '人,有' . $x2 . '  人停止购买社保,有' . $y2 . '  人停止购买商保,有' . $z2 . '  人离职<br/></div>';
			if ($x2)
				$insertInfo .= '<div class="xiaodiv"><br><div class="wenzi">停止购买社保名单:</div>
		<div style=""><table class="myTable"><thead><tr style="text-align:center;">
	    <th>姓名</th><th>编号</th></tr>
	    </thead>
	    <tbody>
	    ...
	    ' . $p2 . '
			...
			if ($z2)
				$insertInfo .= ' <div class="xiaodiv"><br><div class="wenzi">离职名单:</div>
		<div style=""><table class="myTable"><thead><tr style="text-align:center;">
	    <th>姓名</th><th>编号</th></tr>
	    </thead>
	    <tbody>
