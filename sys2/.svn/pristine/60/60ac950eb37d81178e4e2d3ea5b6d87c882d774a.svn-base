...
function wp_handle_upload( &$file, $overrides = false, $time = null ) {
	...
	$new_file = $uploads['path'] . "/" . iconv("UTF-8","GB2312",$filename);//$new_file = $uploads['path'] . "/$filename";
	...
	if ( is_multisite() )
	 ...
	 return apply_filters( 'wp_handle_upload', array( 'file' => $uploads['path'] . "/$filename", 'url' => $url, 'type' => $type ) , 'upload');//return apply_filters( 'wp_handle_upload', array( 'file' => $new_file, 'url' => $url, 'type' => $type ), 'upload' );
}
