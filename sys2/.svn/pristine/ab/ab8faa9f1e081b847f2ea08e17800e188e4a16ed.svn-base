...
<div id="main">
    ...
    <fieldset><legend><code>公积金申报表</code></legend>
                ...
                <tbody>
                    {foreach item=val  from=$ret}
                        <tr>
                            <td>
                                {if $val.status neq '1' }
                                    <input type="checkbox" name="chkList[]" value='{$val.HFModifyDate|cat:"|"|cat:$val.sponsorName|cat:"|"|cat:$val.extraBatch}|{$val.type}'>
                                {/if}
                            </td>
                            <td>
                                {if $val.status eq '1'}<a href="HFListDetail.php?n={$val.sponsorName|escape:'url'}&d={$val.HFModifyDate}&e={$val.extraBatch}&type={$val.type}" target="_blank">
                                        <font color="#FF0000">
                                        {$val.HFModifyDate|substr:5}
                                        </font>
                                    </a>
                                {else}
                                <a href="HFListDetail.php?n={$val.sponsorName|escape:'url'}&d={$val.HFModifyDate}&e={$val.extraBatch}&type={$val.type}" target="_blank">
                                {$val.HFModifyDate|substr:5}
                                    </a>
                                {/if}
                            </td>
