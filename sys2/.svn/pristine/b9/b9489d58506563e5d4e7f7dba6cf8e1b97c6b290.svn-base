...
class soInsFeeAgmInsert {
	...
	public function field() {
		...
		$field = array ("sID", "name", "pID", "radix", "total", "pTotal", "uTotal", "pPension", "uPension" , "pHospitalization", "uHospitalization", "uEmploymentInjury", "uUnemployment", "uPDIns");
	...
	public function validatorSql($getQuery) {
		...
		$sql = "select soInsID from d_soInsFee_tmp where soInsID like '$soInsID' and soInsDate= '$soInsDate' and type='1'";
		$res = $pdo->query ( $sql );
		$row = $res->rowCount ();
		if ($row) {
