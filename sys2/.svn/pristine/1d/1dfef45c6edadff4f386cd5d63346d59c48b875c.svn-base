...
class WxPayApi {
	...
	public function __construct() {
		...
		self::$certPath = $payments ['cert_path'];
		self::$keyPath = $payments ['key_path'];
	}
	/**
	 *
	 * 统一下单
	...
	public static function unifiedOrder($inputObj, $timeOut = 6) {
		...
		if (! $inputObj->IsNotify_urlSet ()) {
			addWeixinLog ( WxPayConfig::NOTIFY_URL, 'notify_url6' );
	...
	private static function reportCostTime($url, $startTimeStamp, $data) {
		...
		// 如果仅失败上报
		if (WxPayConfig::REPORT_LEVENL == 1 && array_key_exists ( "return_code", $data ) && $data ["return_code"] == "SUCCESS" && array_key_exists ( "result_code", $data ) && $data ["result_code"] == "FAIL") {
			return;
		}
