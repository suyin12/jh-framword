...
$title = "业务综合月报表";
if (timeStyle ( "d" ) >= ( insuranceInTurn("soIns")+1))
	$toMon = date ( "Y-m", strtotime (timeStyle ( "Ym" ). "+1 month" ) ) . "-".( insuranceInTurn("soIns")+1);
else
	$toMon = timeStyle ( "Ym" ) . "-".( insuranceInTurn("soIns")+1);
	#获取基本参数
if (! $_GET ['mon'])
...
$smarty->assign ( array ("wCount" => $wCount, "wIncreaseCount" => $wIncreaseCount, "wDecreaseCount" => $wDecreaseCount ) );
$smarty->assign ( array ("cTotalCount" => $cTotalCount, "cIncreaseCount" => $cIncreaseCount, "cRenewalCount" => $cRenewalCount, "cStIncreaseCount" => $cStIncreaseCount, "cOtIncreaseCount" => $cOtIncreaseCount ) );
$smarty->assign ( array ("sInsCount" => $sInsCount, "sInsIncreaseCount" => $sInsIncreaseCount, "sInsDecreaseCount" => $sInsDecreaseCount ) );
$smarty->assign ( array ("cInsCount" => $cInsCount ) );
$smarty->assign ( array ("heIncreaseCount" => $heIncreaseCount, "heCount" => $heCount ) );
$smarty->assign ( array ("szCount" => $szCount, "nszCount" => $nszCount ) );
#模板配置
$smarty->assign ( array ("title" => $title, "css" => httpPath . "css/main.css", "httpPath" => httpPath ) );
$smarty->display ( "leader/performanceSummary.tpl" );
