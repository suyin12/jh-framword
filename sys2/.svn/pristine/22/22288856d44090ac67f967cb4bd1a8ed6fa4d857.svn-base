...
if ($_GET ['year']) {
	...
	header ( "location:" . httpPath . "salaryManage/salaryIndex.php?year=" . date ( "Y", time() ) );
}
...
#获取相关数据
$sql = "select unitID,month,salaryDate,zID,comInsDate,soInsDate,managementCostDate from a_originalFee_tmp where substr(month,1,4)= :year ";
$sql = $sql . $sqlUnit;
...
$smarty->display ( "salaryManage/salaryIndex.tpl" );
