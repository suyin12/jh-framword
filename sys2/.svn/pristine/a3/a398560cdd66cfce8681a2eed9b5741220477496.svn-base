...
class WikiNotifications {
	...
	function page_edit_notification($pageID) {
	    ...
	    if($wpw_options['email_admins'] == 1){
			...
			$message = sprintf(__("帮助文档已经修改: %s."),get_option('home'),$pageTitle);
			$message .= "\n\r";
			$message .= sprintf(__("页面名 : %s"), $pageTitle); 
			$message .= "\n\r";
			$message .= __('查看该页面:, ').'<a href='.$pagelink.'>'.__('点击查看').'</a>';
	    ...
	    if ($wpw_options['cron_email'] == 1) {
	        ...
	        if ($results) {
	            foreach ($results as $result) {
	                ...
	                $message .= "页面标题为  ".$pageTitle.". \n\r 查看该页面 <a href='".$pagelink."'> 点击查看</a>.\n\r\n\r";
	                //exit(print_r($emails, true));
	                foreach($emails as $email){
	                    wp_mail($email, $subject, $message);
	                }
