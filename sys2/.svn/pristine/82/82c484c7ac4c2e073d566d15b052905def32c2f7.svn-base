...
//                }
    ...
    public function fillData($tableTT) {
        $eRes = $this->eRes;
        $selFieldArray = $this->selFieldArray;
...
#设置标题栏在第几行及它的 行高(用作标题行)
        ...
        #表头表尾
        $keysArr = array_keys($eRes);
        $keysEnd = end(array_keys($tableTT));
        $footkey = end($keysArr)+ $headRow;
        $tt = array(
            'A'.$footkey.':'.'B'.$footkey => '合计',
        );
        $tableTT = array_merge($tableTT, $tt);
        if(!empty($tableTT))
        {
            foreach($tableTT as $k => $v){
                //合并单元格
                if(strpos($k,':')){
                    $oSheet->setCellValue(substr($k,0,strpos($k,':')),$v);
                    $oSheet->mergeCells($k);
                }else{
                    $oSheet->setCellValue($k,$v);
                }
            }
            //设置单元格样式
            $objArr = array(
