...
class WeixinAddonModel extends WeixinModel{
	function reply($dataArr, $keywordArr = array()) {
		...
		foreach($keywordsValue as $v){
			...
			if($ispass){
				if($v["zdtype"] == 0){
// 					addWeixinLog($v,'testajax111111');
					$this->transmitServiceZD($dataArr,$v["msgkfaccount"]);
// 					$nl=strlen($v['msgkfaccount'])-strlen('@cnhrmo');
// 					$kfnickname=substr($v['msgkfaccount'], 0,$nl);
// 					$data=$this->replyText('您好，为你接入客服');
					$uid=M('public_follow')->where(array('openid'=>$dataArr['FromUserName']))->getField('uid');
					$this->send_message($v,$uid);
    ...
    //公众号推送信息
    function send_message($data,$uid){
    	$customDao=D('Common/Custom');
    	$type=$data['type'];
    	switch ($type){
    	    case 1:
    	    	if ($data['msg_test']){
    	    		$customDao->replyText($uid,$data['msg_test']);
    	    	}else{
    	    		$customDao->replyText($uid,'你好，正在转接中...');
    	    	}
    	    	//文本
    	    	break;
    	    case 2:
    	    	// 图片
    	    	$image_material = $data['image_id'];
    	    	if ($image_material) {
    	    	    $imageMaterial = M ( 'material_image' )->find ( $image_material );
    	    	    if ($imageMaterial ['media_id']) {
    	    	       $mediaId = $imageMaterial ['media_id'];
    	    	    } else {
    	    	       $mediaId = $customDao->get_image_media_id ( $image_material['cover_id'] );
    	    	    }
    	    	    $result = $customDao->replyImage ( $uid, $mediaId, '' );
    	    	} else {
    	    	   $customDao->replyText($uid,'你好，正在转接中...');
    	    	}
    	    	//图片
    	    ...
    	    case 3:
    	    	//图文
    	    	$groupId= $data ['appmsg_id'];
    	    	if (!empty ( $groupId )) {
    	    	   $result = $customDao->replyNews ( $uid,$groupId );
    	    	}else{
    	    		$customDao->replyText($uid,'你好，正在转接中...');
    	    	}
    	    	break;
