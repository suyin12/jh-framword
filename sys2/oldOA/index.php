...
$OpIDs=trim($judgeRow['OpIDs'],',');
//echo "222=".$OpIDs;
echo "<div class='tishi'>";
    if($_SESSION['Cqyyh']==13&&strpos("$OpIDs",'55')!='')
	{
	 require("../dbs.php");
  $link= new mysql();
...
////查询所有未审核数据
$value_shenhe=$link->findall_cnhrmo_all(cmsware_publish_119,shenhe,0,1,$date);//(表名，是否申请字段名，是否申请值，要删除的天数，当前日期)
$vol_shenhe=$link->db_num_rows($value_shenhe);
//print_r($vol_shenhe);
	$sh=$vol_shenhe;
	echo "今日有&nbsp<font color='#FF0000'>".$sh."</font>&nbsp家待审核公司！";
	}
echo "</div>";
...
?>
<form style="float:left" action="workerManager/fixedWorker.php"  target='_self' name="TTT" id="TTT" method="post"> 
...
<br />
 ...
 <div class="accordion">
 <h3>员工生日提醒</h3>
 <table class="tablesorter"   border="0" cellspacing="1" cellpadding="2" bgcolor="#666666">
 <thead>
 <tr bgcolor="#CAE8EA">
 <th>单位名称</th>
 <th>部门名称</th>
...
//		echo "<p style='color:red'>身份证号码位数错误信息提示:姓名:".$birthdayRow[field6]."&nbsp&nbsp身份证号码:".$birthdayRow[field7]."&nbsp&nbsp单位:".$birthdayRow[field3]."</p>";
 ...
 ?>
	...
	<td class="edit"><a href='workerManager/workerInfo.php<? echo "?"; ?>sessionID=<?php echo $birthdayRow['sessionID'];?>&IDNumber=<?php echo $birthdayRow['field7'];?>'><? echo $birthdayRow['field6']; ?></a></td>
	<td><? echo $birthdayRow['field7'];?></td>
	<td><? echo $age;?></td>
	<td><? echo $birthdayRow['ield16'];?></td>
...
//			print_r($birRow);
		...
		<tbody>
		<?php 
		$undermannedSql="select * from undermannedView group by field4 where sessionID='$sessionID'";
...
//		    print_r($undermannedRow);
		...
		<td class="edit"><a href='workerManager/workerInfo.php<? echo "?"; ?>sessionID=<?php echo $undermannedRow['sessionID'];?>&salaryNo=<?php echo $undermannedRow['field4'];?>&workerName=<?php echo $undermannedRow['field3'];?>'><? echo $undermannedRow['field3']; ?></a></td>
