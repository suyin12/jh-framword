...
#
$feeSql = "select * from `a_mul_originalFee_tmp` where month like :month and unitID like :unitID and extraBatch =:extraBatch limit 1";
...
unset($feeRet);
#如果存在调账记录, 则以 a_mul_originalFee_tmp 的实收,为准
$oSql = "select a.uID,(a.uPension+a.uHospitalization+a.uEmploymentInjury+a.uUnemployment+a.uBirth) as uSoIns,a.uHF,uComIns,a.managementCost,a.uPDIns from `a_mul_originalFee_tmp` a  where a.month like :month and a.unitID like :unitID and a.extraBatch =:extraBatch  and a.modifyType='1'";
$oRet = SQL($pdo, $oSql, array(":month" => $month, ":unitID" => $unitID));
$oRet = keyArray($oRet,'uID');
// print_r($oRet);
#
$selSql = "select a.*,b.status,b.soInsModifyDate,b.soInsBuyDate,b.HFModifyDate,b.housingFund,b.soInsurance from a_fee_tmp a left join a_workerInfo b on a.uID=b.uID  where a.month like :month and a.unitID like :unitID and a.extraBatch=:extraBatch";
if ($_POST['search']){
...
$smarty->display("salaryManage/feeEdit_mul.tpl");
?>