...
abstract class Controller {
	...
	private function initUser() {
		...
		if (! $uid) {
			$uid = M ( 'config' )->where ( 'name="FOLLOW_YOUKE_UID"' )->getField ( 'value' ) - 1;
			$user ['uid'] = $uid;
			M ( 'config' )->where ( 'name="FOLLOW_YOUKE_UID"' )->setField ( 'value', $uid );
			session ( 'mid', $uid );
		}
	...
	private function initMoblie($info) {
		...
		$in_weixin = isWeixinBrowser ();
		if (IS_GET && $in_weixin && $is_need_bind && $userNeed && C ( 'USER_OAUTH' )) {
			Cookie ( '__forward__', $_SERVER ['REQUEST_URI'] );
	...
	/*
	 * 在大并时优化插入数据性能，该函数需要两次调用： 一是在插入数据的地方调用，这时table和data两个参数不能为空 二是管理在后台查看列表时调用，这时table参数不为空，但data为空，这些防止有部分数据在缓存里无法到达数据库
	 */
