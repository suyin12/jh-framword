...
class MaterialController extends HomeController {
	...
	function material_lists() {
		...
		$list = M ( 'material_news' )->where ( $map )->field ( $field . ',count(id) as count' )->group ( 'group_id' )->order ( 'group_id asc' )->page ( $page, $row )->select ();
		foreach ( $list as &$vo ) {
			if ($vo ['count'] == 1)
				continue;
		...
		$this->assign ( 'add_url', U ( 'add_material' ,array('mdm'=>I('mdm'))) );
		$this->display ();
	}
	...
	function material_data() {
		...
		$field = 'id,title,cover_id,intro,group_id';
		$list = M ( 'material_news' )->where ( $map )->field ( $field . ',count(id) as count' )->group ( 'group_id' )->order ( 'group_id asc' )->select ();
