...
class hfFeeAgmInsert {
    ...
    public function validatorSql($getQuery) {
        ...
        $sql = "select housingFundID from d_hffee_tmp where housingFundID like '$housingFundID' and HFDate= '$HFDate' ";
        $res = $pdo->query($sql);
        $row = $res->rowCount();
        if ($row) {
    ...
    public function sql() {
        ...
        foreach ($cellArray as $cValue) {
            ...
            $s_V = rtrim($s_V, ",");
            $sqlV = "(" . $s_V . ")";
            $sql [] = "insert into d_hffee_tmp ( " . $sqlK . ") values " . $sqlV;
        }
        return $sql;
    }
