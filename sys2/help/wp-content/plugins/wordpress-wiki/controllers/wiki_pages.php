...
class WikiPageController {
    ...
    function post_revisions() {
        ...
        if ($this->WikiHelper->is_wiki('front_end_check')) {
            ...
            $date = date(__('Y/m/d g:i a'), mktime($post->post_modified));

            $author = $this->WikiHelper->get_author($post);

            $latest_revision = sprintf(__('最新修订版 (@ %1s by %2s)'), $post->post_modified, $author);
            ...
            if ($revisions) {
                ...
                foreach ($revisions as $revision) {
                    if (@wp_get_post_autosave($post->ID)->ID != $revision->ID) {
                        ...
                        $revision_title = sprintf(__('版本 @ %1s by %2s'), $date, $author);
    ...
    function table_of_contents($content) {
        ...
        return "<div id='wikiTable' class='contents alignright'><h3>" . __('提纲') . "</h3><p> &#91; <a class='show' onclick='toggle_hide_show(this)'>" . __('隐藏') . "</a> &#93; </p>$table</div>" . $content;
    }
    ...
    function set_query() {
        if (isset($_GET['revision'])) {
            ...
            $warning = '<div id="wpw_read_revision_warning">' . __('当前版本 ') . ' @ ' . $revision_data->post_modified . ' ' . __('by') . ' ' . $revision_author->display_name . '. <a href="' . get_permalink($post->ID) . '">' . __('Current version') . '</a></div>';
    ...
    function wpw_nope($content) {
        ...
        $message = __(' ');
        $message .= '&nbsp;<a href="' . wp_login_url(get_permalink($post->ID)) . '">' . __('') . '</a>';
    ...
    function get_content($content, $class = null) {
        ...
        $children = get_children('post_parent=' . $post->ID . '&post_type=wiki');
        if ($children):
            $i = $k=0;
            $bottom = '';
            foreach ($children as $child) {
                if ($i == 0) {
                    $bottom .='<div class="wikiList"><h2>' . $child->post_title . '</h2><ul>';
                    $children_Lv2 = get_children('post_parent=' . $child->ID . '&post_type=wiki');
                    foreach ($children_Lv2 as $child_Lv2) {
                        $bottom .= '<li><a href="' . get_permalink($child_Lv2->ID) . '" >' . $child_Lv2->post_title . '</a></li>';
                    }
                    $bottom .="</ul></div>";
                    $bottom .="<div class='clear'></div>";
                } else {
                    $bottom .='<div class="wikiList"><h2>' . $child->post_title . '</h2>';
                    $children_Lv2 = get_children('post_parent=' . $child->ID . '&post_type=wiki');
                    $children_Lv2Num = count($children_Lv2);
                    //如果超过5个项目,则分栏显示ul
                    if ($children_Lv2Num > 5) {
                        $j = 0;
                        $bottom .='<div class="left-col"><ul>';
                        foreach ($children_Lv2 as $child_Lv2) {
                            if ($j > 4 && $j % 5 == 0)
                                $bottom .="</ul></div><div class='left-col'><ul>";
                            $bottom .= '<li><a href="' . get_permalink($child_Lv2->ID) . '" >' . $child_Lv2->post_title . '</a></li>';
                            $j++;
                        }
                        $bottom .="</ul></div></div><div class='clear'></div>";
                    } else {//不超过5个正常输出
					    $k++;						
                        $bottom .='<ul>';
                        foreach ($children_Lv2 as $child_Lv2) {
        ...
        $content.='<p style="text-align:center;"><a href="#wikiTable"><<   查看提纲</a>   | <a href="?wiki=帮助文档" >帮助文档首页   >></a></p>';
        return '<div id="wpw_read_div" ' . $class . '>' . $this->table_of_contents(wptexturize($this->wiki_parser($content))) . '</div><div class="clear"></div>';
    }
    ...
    function front_end_interface($content) {
        ...
        return
			...
			<div id="wpw_tabs">
			<ul id="wpw_tab_nav">
				<li><a id="wpw_read" href="?wpw_action=content">阅读</a></li>
				<li><a id="wpw_edit" href="?wpw_action=edit">编辑</a></li>
				<li><a id="wpw_view_history" href="?wpw_action=history">查看历史</a></li>
			</ul>
			' . $warning
                . $return . '
			</div>
