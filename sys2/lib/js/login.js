...
$(document).ready(function(){
    ...
    var iCodeId ="#iCode";
    var waitNote = '载入中...'; // loading message
    var jsErrMsg = '用户名或密码不正确'; // clientside error message
    ...
    $(waitId).html(waitNote).fadeIn('fast', function(){
        ...
        $.getJSON(postFile, function(data){
            if (data.status == true) {
                ...
                if (autoRedir) {
                    $(waitId).hide().html('Redirecting...').fadeIn('fast', function(){
                                    ...
                                    $.post(postFile, {
                                        ...
                                        if (data.status == true) {
                                            ...
                                            $(waitId).html(data.message).slideDown('fast', function(){
                                                $("#imCode").attr("src","identifyCode.php?tag=2");
                                                $(userId).focus();
                                            });
                                        }
                                    }, 'json');
