...
switch ($whatDate) {
    case "rewardDate" :
        ...
        $zID = $_GET['zID'];
        $sql = "select bID,spID,unitID,name from `a_rewardFee_tmp` where unitID like :unitID and month like :month and extraBatch=:extraBatch and uID like ''";
        $res = $pdo->prepare($sql);
...
foreach ($reCountFeeName as $reCFNKey => $reCFNVal) {
    if ($reCFNVal > 1)
        $errMsg [] = "同一个单位中,原始费用表中存在相同姓名{<a href='" . httpPath . "rewardManage/detail.php?a=reward&month=" . $month . "&unitID=$unitID&extraBatch=$extraBatch&zID=$zID&m=name&c=$reCFNKey' target='_blank'>" . $reCFNKey . "</a>}的员工,请修改,使之与费用表中的姓名一致并得以区分";
}
#验证有工资账号的在不在花名册中,满足的条件是,姓名及其工资账号必需吻合
if ($bIDArr_Fee) {
    if ($bIDArr_wInfo)
        $bIDErrArr = array_diff_assoc($bIDArr_Fee, $bIDArr_wInfo);
    else
...
if ($bIDErrArr || $nameErrArr) {
    if ($bIDErrArr)
        foreach ($bIDErrArr as $bEKey => $bEVal) {
            $errMsg [] = "花名册中不存在特定编号为{<a href='" . httpPath . "rewardManage/detail.php?a=reward&month=" . $month . "&unitID=$unitID&extraBatch=$extraBatch&zID=$zID&m=spID&c=$bEKey' target='_blank'>" . $bEKey . "</a>/<a href='" . httpPath . "workerInfo/wInfo.php?m=name&c=" . $bEVal . "' target='_blank'>" . $bEVal . "</a>}或姓名与花名册不匹配";
        }
    if ($nameErrArr)
        foreach ($nameErrArr as $nEKey => $nEVal) {
            $errMsg [] = "花名册中不存在该员工信息{<a href='" . httpPath . "rewardManage/detail.php?a=reward&month=" . $month . "&unitID=$unitID&extraBatch=$extraBatch&zID=$zID&m=name&c=$nEVal' target='_blank'>" . $nEVal . "</a>}";
        }
} elseif (!$errMsg) {
    switch ($whatDate) {
        case "rewardDate" :
            if ($bIDArr_Fee)
                foreach ($bIDArr_Fee as $bID => $name) {
                    $updateSql [] = "update a_rewardFee_tmp x , a_workerInfo y set x.uID=y.uID  where  x.uID like '' and x.spID=y.spID and x.month like '$month' and x.extraBatch like '$extraBatch'  and x.spID like '$bID'";
                }
            if ($nameArr_Fee)
                foreach ($nameArr_Fee as $name) {
                    $updateSql [] = "update a_rewardFee_tmp x , a_workerInfo y set x.uID=y.uID  where x.uID like '' and x.name=y.name and x.unitID=y.unitID and x.unitID like '$unitID'  and x.month like '$month' and x.extraBatch like '$extraBatch'  and x.name like '$name'";
                }
            break;
            break;
    }
...
$smarty->display("rewardManage/validOriginalReward.tpl");
?>