...
function encryptDecrypt($key, $string, $decrypt){
    if($decrypt){
        $decrypted = rtrim(openssl_decrypt(MCRYPT_RIJNDAEL_256, md5($key), base64_decode($string), MCRYPT_MODE_CBC, md5(md5($key))), "16");
        return $decrypted;
    }else{
        $encrypted = base64_encode(openssl_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $string, MCRYPT_MODE_CBC, md5(md5($key))));
        return $encrypted;
    }
}
//以下是将字符串“Helloweba欢迎您”分别加密和解密
//加密：
echo encryptDecrypt('password', 'Helloweba欢迎您',0);echo "<br>";
//解密：
 ...
 *
 * Utf-8、gb2312都支持的汉字截取函数cut_str(字符串, 截取长度, 开始长度, 编码);编码默认为 utf-8开始长度默认为 0
 *
 */
...
function message($msgTitle,$message,$jumpUrl){
    ...
    $str .= "<script>setTimeout('location.replace(\'".$jumpUrl."\')',10000)</script>";
    $str .= '</div>';
    $str .= '</div>';
    $str .= '</body>';
    $str .= '</html>';
...
//message('操作提示','操作成功！','http://baidu.com');